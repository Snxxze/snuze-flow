...
export const KanbanBoard: React.FC<KanbanBoardProps> = ({
  ...
  const renderTaskCard = (tObj: Task) => {
    ...
    return (
      <div
        key={tObj.id}
        className="group rounded border border-surface-border bg-surface p-3 transition-colors hover:border-charcoal/30 cursor-pointer"
      ...
      >
        ...
        {/* Description */}
        {tObj.description && (
          <p className="mt-1 text-[11px] text-charcoal-subtle line-clamp-2 leading-relaxed">
            {tObj.description}
          </p>
        )}
        ...
        {/* Metadata row */}
        <div className="mt-2.5 flex flex-wrap items-center justify-between gap-y-1 gap-x-2">
        ...
        {isTaskExpanded && (
          <div
            ...
            className="mt-2.5 pt-2.5 border-t border-surface-border space-y-1"
          >
            {isSubtaskLoading ? (
        ...
        <div className="grid grid-cols-1 gap-0 md:grid-cols-3 md:divide-x md:divide-surface-border min-h-[400px]">
          {columns.map((col) => {
            ...
            return (
              <div
                key={col.id}
                className={`flex flex-col px-4 first:pl-0 last:pr-0 transition-all ${
              ...
              >
                ...
                {!collapsed && (
                  <div className="flex-1 space-y-2 overflow-y-auto max-h-[calc(100vh-300px)]">
                    {colTasks.length === 0 ? (
                      <p className="text-[11px] text-charcoal-subtle/40 py-3 text-center">
                        ยังไม่มีงาน
                      </p>
                    ) : (
